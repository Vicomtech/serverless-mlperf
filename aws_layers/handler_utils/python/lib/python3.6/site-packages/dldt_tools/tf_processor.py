...
class TFProcessor:
    def __init__(self):
        super(TFProcessor, self).__init__()
        self.inputs = []
        self.outputs = []
        self.net = None
        self.image_src = None
    ...
    def load(self, model_path, inputs=None, outputs=None, make_profiling=False):
        ...
        if model_path.endswith(".pbtxt"):
            ...
            if os.path.exists(weight_model):
                ...
                try:
                    if self.make_profiling:
                        t0 = time.time()
                    if "ssd" in weight_model:
                        self.net = cv2.dnn.readNetFromTensorflow(weight_model, config_model)
                    else:
                        print("before reading")
                        self.net = cv2.dnn.readNet(weight_model)
                        print("after reading model {}".format(weight_model))
            ...
            raise FileNotFoundError("File not found please provide .pb file")
        return self
    ...
    def load_input(self, image, input_shape):
        ...
        if len(self.config["mean"]):
            ...
            self.blob = cv2.dnn.blobFromImage(input_image, size=(input_shape[1], input_shape[2]), swapRB=False)
        if self.make_profiling:
            t1 = time.time()
            performance_in_ms = 1e3 * (t1 - t0)
